...
- **목적:** 
  ...
  2. 단순 문법 나열을 넘어 나이브한 베이스라인의 한계와 구조적 리팩터링 과정을 담은 기술 의사결정 기록(ADR)을 작성합니다.
  3. 평일 17:20 Antigravity 정기 스케줄러가 커리큘럼 명세와 로컬 코드를 인제스트하여 초안을 대기시키고, 사용자의 컨펌을 거쳐 Astro 정적 블로그로 배포합니다.
...
sequenceDiagram
    autonumber
    actor Dev as 엔지니어
    participant AGY as Antigravity (Scheduled Task 17:20)
    ...
    Note over AGY: 평일 17:20 정기 크론 자동 기상
...
강의를 직접 듣지 않아도 엔지니어링 맥락을 파악할 수 있도록 구성합니다:
1. **나이브한 베이스라인:** 튜토리얼 수준의 단순 구현 예제 스니펫(5~10줄)을 먼저 제시.
2. **실무 결함과 문제의식:** 대용량 트래픽이나 운영 환경에서 해당 코드가 실패하는 구체적 이유(비정형 데이터, 메모리 누수, 예외 미처리)를 분석.
3. **초기 프로토타입 vs 리팩터링 구현체:** 1차 시도 코드(`*practice.py`)와 LLM 기반 구조 최적화 코드(`*llm.py`)를 대조 분석 (*llm.py가 없는 경우 주석 기반 구조 개선안으로 단독 서술).

### 2) 커리큘럼 세부 실습 명세 자동 인제스트 (Zero-Effort)
컨텍스트 로더(`context_loader.py`)로 메인 학습 목표와 세부 실습 이론, 정답 코드, 트러블슈팅 Q&A를 실시간 바인딩합니다. 수동 요약 노트(`lecture_notes.md`)를 작성할 필요가 없습니다.

### 3) 품질 검증 체계와 결정론적 하네스 (`harness.py`)
- **이모지 배제:** 본문, 제목, 커밋 메시지 어디에도 유니코드 이모지를 쓰지 않습니다.
- **불필요한 괄호 영어 차단:** `손상(Broken)` 같은 번역투 괄호 영어를 막고, `IDS`, `SOAR`, `REST API`, `PCAP` 등 공인 대문자 표준 약어만 허용합니다.
- **비전문적 메타 어휘 차단:** `수강생`, `페어 프로그래밍`, `학습자`, `모범 답안`, `교시` 등 학생 티를 내는 어휘를 하네스 차원에서 물리적으로 차단합니다.
- **자립형 룰북:** `im-not-ai` 72대 룰북을 레포지토리 내부에 포함해 일관된 품질을 엄격히 유지합니다.
