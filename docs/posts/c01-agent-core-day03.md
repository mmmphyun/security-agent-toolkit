...
`detect_llm.py`를 실행하여 `sample_server.log`를 분석한 콘솔 출력 결과는 다음과 같다.

```text
[요약] 파싱 17건 / 건너뜀 5건
[룰 1] 확인 필요: admin — 실패 5회
[룰 2] 의심 IP: 185.220.101.34 — 계정 4개 시도: [guest, oracle, root, test]
[룰 3] 심야 접속: 03:17:09 admin (211.45.12.9)
```

인증 로그 17건을 정상 파싱하고 비인증 커널/시스템 로그 5건을 안전하게 건너뛰었다. 탐지 엔진은 `admin` 계정에 대한 5회 연속 실패, `185.220.101.34` IP에서의 4개 계정 분산 시도(Password Spraying), `03:17:09` 심야 시간대의 비인가 로그인을 각각 정확하게 식별하여 `alerts.json` 파일에 저장했다.
