...
보안 관제와 침해사고 분석의 출발점은 비정형 텍스트 형태로 수집되는 시스템 로그를 정형 데이터로 변환하는 작업이다. 운영체제, 웹 서버, 네트워크 장비가 남기는 접근 로그는 표준 규격을 완벽히 따르지 않거나 필드 간 공백 길이가 불규칙한 경우가 흔하다. 원시 로그에서 위협 징후를 식별하려면 데이터 전처리 단계에서 공백 불규칙성을 정규화하고 필드를 안전하게 분리해야 한다.

반복적인 로그인 실패 이벤트는 비인가자의 계정 탈취 시도인 무차별 대입 공격의 대표적인 지표다. 다량의 로그 스트림에서 계정별 실패 빈도를 빠르게 집계하기 위해 파이썬 표준 라이브러리인 `collections.Counter`와 중복을 배제하는 `set` 자료형을 활용한다. 발생 건수(`len(failed_users)`)와 실제 피해 대상 식별자 수(`len(set(failed_users))`)를 분리하여 측정하는 것이 정확한 위협 수치 산출의 기본이다.
...
flowchart LR
    ...
    B --> C["정형 딕셔너리 리스트"]
    C --> D["find_suspects (login_failed 필터링)"]
...
강의 교재의 기본 베이스라인 코드는 다음과 같은 단순 루프 형태로 제시된다.
...
이 방식은 다음과 같은 실무적 결함을 안고 있다.

1. **고정 구분자 분할로 인한 토큰 왜곡:** 로그 생성 환경에 따라 필드 사이의 공백은 단일 스페이스, 다중 스페이스, 탭 문자(`\t`)가 혼용된다. `line.split(" ")`처럼 단일 공백을 고정 구분자로 쓰면 연속 공백이 빈 문자열(`""`) 토큰으로 파싱되어 인덱스가 밀리는 데이터 오염이 발생한다.
...
이러한 한계점을 해결하기 위해 `hello.py`를 작성하며 세 가지 핵심 엔지니어링 개선을 적용했다.
...
`hello.py`를 실행하여 샘플 로그 20건을 주입한 결과는 다음과 같다.
