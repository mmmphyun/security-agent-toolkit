...
실제 RPG Sync 프로젝트에서 객체 스토리지인 Cloudflare R2를 연동할 때도 이 원리가 적용되었다. 개발 환경의 dev 도메인이 정적 자산 서비스용 `assets` 도메인을 별칭으로 참조하도록 CNAME을 구성했을 때, 프록시 계층이 이를 에지 IP로 즉시 평탄화하여 추가 질의 지연 없이 직결시켰다.
