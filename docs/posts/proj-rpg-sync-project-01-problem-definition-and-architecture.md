...
또한 `src/database/cache.py`에서는 Redis 연결 실패 시 프로세스가 다운되지 않고 즉시 DB 직접 조회로 폴백(Fail-Open)하는 **서킷 브레이커**와 백그라운드 PING을 통한 **자가 치유** 로직을 구현하여 캐시 장애 상황에서도 전체 서비스 가용성을 보장하도록 설계했다.
